...
int split_pipe(csalt_store *fd, void *param)
{
	...
	if (result != 0) {
		print_error("csalt_store_size() returned unexpected result for split pipe: %d", result);
		exit(EXIT_FAILURE);
	}
	return EXIT_SUCCESS;
}
...
int main()
{
	...
	if (pipe_size != 0) {
		print_error("Unexpected csalt_store_size() value for pipe: %d", pipe_size);
		return EXIT_FAILURE;
	}
	...
	if (real_file_size != 10) {
		print_error("Unexpected csalt_store_size() value for file: %d", real_file_size);
		return EXIT_FAILURE;
	}

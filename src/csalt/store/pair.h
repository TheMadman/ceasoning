...
/**
 * \file
 * \brief This file provides store which define relationships
 * between store. Examples include csalt_store_pair, which can be used
 * to create pairs, linked-lists or binary trees of store, and
 * csalt_store_fallback, which allows you to define store as a priority
...
/**
 * \brief This type allows storing a pair of store and interacting
 * 	with them as though they are a single store.
 *
 * This type can be used to represent coupled store, as well as lists
 * of store using a linked-list-like approach or binary trees using
 ...
 * as one for initializing an array of pairs for a list of store.
 *
 ...
 * the write was successful on both store, the pair returns the lowest
 ...
 * \code
 * 	struct csalt_store_pair list[arrsize(store)] = { 0 };
 * \endcode
 *
 ...
 *
 * \param begin The beginning of the array of store
 * \param end The end of the array of store
...
/**
 * \brief Split multiple store using different bounds per store.
 *
 * This function takes a linked list of store, as constructed by
 ...
 *
 * Once the store are split, they are wrapped in a new linked list
 * and passed to `block`.
 *
 * Example:
 *

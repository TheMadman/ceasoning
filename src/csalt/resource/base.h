...
/**
 * \file
 * \brief Provides an interface for resource with lifetimes.
 *
 * This file provides both a common interface for resource which
 * require clean-up; a function to manage a resource automatically;
 * and a resource for managing heap memory.
 *
 * Note that resource do not _necessarily_ do bounds checking -
 * for some resource, writing beyond the end of them is a valid
 ...
 * after.
 *
 * To create custom structs which can manage resource,
 * use a struct csalt_dynamic_resource_interface* as the first
 * member.
 *
 * Functions operating on resource take a csalt_resource
 ...
 * allows lazy evaluation of resource
 */
...
/**
 * \brief Interface definition for managed resource.
 *
...
/**
 * \brief Interface definition for managed resource which
 * 	return static (non-resizable) store.
 *
 * Structs with a pointer-to-resource-interface
 * as their first member can be passed to resource
 * functions with a simple cast.
 ...
 *
 * This function is used for csalt_static_resource as well.
 */
